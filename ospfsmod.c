...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		 ...
		 * EXERCISE: Your code here */
		 int byteOffset = (f_pos-2) * OSPFS_DIRENTRY_SIZE; 

		 if(byteOffset >= dir_oi->oi_size) 
		 {
			r = 1;	
			break;		
		 }
		...
		/* EXERCISE: Your code here */
		 ...
		 //treat all entries in directory file as a "sequence of ospfs_direntry structures".
		 od = (ospfs_direntry_t*) ospfs_inode_data(dir_oi, byteOffset);

		 //get the corsponding inode for this sub file
...
ospfs_unlink(struct inode *dirino, struct dentry *dentry)
{
	...
	if (oi->oi_nlink==1 && oi->oi_ftype != OSPFS_FTYPE_SYMLINK) {
        change_size(oi, 0); //deallocate all data block assciated with this inode
    }

	od->od_ino = 0; //mark inode as free
	oi->oi_nlink--;
...
{
	/* EXERCISE: Your code here */
	//i is the starting block no traverser
	int bitMapStartBlock = OSPFS_FREEMAP_BLK;
	int bitMapEndBlcok = ospfs_super->os_firstinob-1;
	int i; 
	for(i = bitMapStartBlock; i <= bitMapEndBlcok; i++)
	{
		void* currBlockArray = ospfs_block(i);
		int j;
		for(j = 0; j < OSPFS_BLKBITSIZE; j++)
		{
			 if (bitvector_test(currBlockArray, j)==1) 
			 {
                bitvector_clear(currBlockArray, j); 
                return (i - bitMapStartBlock) * OSPFS_BLKBITSIZE + j;
             }
		}
	}
...
free_block(uint32_t blockno)
{
	...
	int bitMapStartBlock = OSPFS_FREEMAP_BLK;
	int bitMapEndBlcok = ospfs_super->os_firstinob-1;
	//we want to jump over the book-keeping info blocks
	if(blockno < ospfs_super->os_firstinob + ospfs_super->os_ninodes) 
		return;
	int i = bitMapStartBlock + blockno / OSPFS_BLKBITSIZE; //which block of bitmap
	int j = blockno % OSPFS_BLKBITSIZE; //which index in that particular bitmap array
	bitvector_set(ospfs_block(i), j);
	return;
}
...
{
	// Your code here.
	// OSPFS_NDIRECT: # of direct block pointers
	// OSPFS_NINDIRECT: # of block pointers in an indirect block.
	if(b < OSPFS_NDIRECT)
		return -1;
	if(b < OSPFS_NDIRECT + OSPFS_NINDIRECT)
		return 0;
	//need to use double indrect
	//return the index of the UNDIRECT BLOCK in the double indrect array
...
{
	// Your code here.
	int32_t r = indir_index(b);
	if(r == -1)
		return b;
	if(r == 0)
		return b - OSPFS_NDIRECT;
	//r is the index of the UNDIRECT BLOCK in the double indrect array
	//return the index on the final indrect block that points to block b
	return b - OSPFS_NDIRECT - OSPFS_NINDIRECT - OSPFS_NINDIRECT * (r);
}
...
add_block(ospfs_inode_t *oi)
{
	...
	//pre-allocate the data block
	uint32_t newDataBlock = allocate_block();
	if(newDataBlock == 0)
		return -ENOSPC;
	memset((void*)ospfs_block(newDataBlock), 0, OSPFS_BLKSIZE); //zero-out the newBlock
	allocated[0] = &newDataBlock;
	uint32_t newIndex = n; //absolute index for the newly to-be-add block, 0 based


	//if there is any empty direct block? 
	if(indir_index(newIndex) == -1)
	{
		oi->oi_direct[newIndex] = newDataBlock;
	...
	//indrect block
	else if(indir_index(newIndex) == 0)
	{
		int indirectIndex = direct_index(newIndex);
		if(indirectIndex == 0) //if this is our first indrect entry, allocate the indrect block
		{
			int newIndirectBlock = allocate_block();
			if(newIndirectBlock == 0)
			{
				free_block(newDataBlock); //back track and free data block
				return -ENOSPC;
			}
			memset((void*)ospfs_block(newIndirectBlock), 0, OSPFS_BLKSIZE); //zero-out the newBlock
			oi->oi_indirect = newIndirectBlock;
		}
		uint32_t* indirectBlockArray = (uint32_t*) ospfs_block(oi->oi_indirect);
        indirectBlockArray[direct_index(newIndex)] = newDataBlock;
        oi->oi_size += OSPFS_BLKSIZE;
        return 0;
	}
	...
	else if(indir2_index(newIndex) == 0)
	{
		uint32_t* doubleIndirectArray;
		uint32_t* indirectArray;
		//first check if double indirect block is initialized:
		if(indir_index(newIndex) == 0 && direct_index(newIndex) == 0)
		{
			...
			memset((void*)ospfs_block(oi->oi_indirect2), 0, OSPFS_BLKSIZE); //zero-out the newBlock
		}
		//Then check if the indrect block entry on the double indirect block is allocated
		else if(direct_index(newIndex) == 0)
		{
			doubleIndirectArray = (uint32_t*) ospfs_block(oi->oi_indirect2);

			uint32_t newIndirectBlock = allocate_block();
            if (newIndirectBlock==0) 
            {
                //deallocate newly created double indirect block
                if (indir_index(newIndex) == 0 ) 
                {
                    free_block(oi->oi_indirect2);
                    oi->oi_indirect2 = 0;
                }
                free_block(newDataBlock);
                return -ENOSPC;
            }
            doubleIndirectArray[indir_index(newIndex)] = newIndirectBlock;
		}

		indirectArray = (uint32_t*) ospfs_block(doubleIndirectArray[indir_index(newIndex)]);
		indirectArray[direct_index(newIndex)] = newDataBlock;
		oi->oi_size += OSPFS_BLKSIZE;
		return 0;
	}
...
remove_block(ospfs_inode_t *oi)
{
	...
	/* EXERCISE: Your code here */
	uint32_t lastIndex = n - 1;
	bool useDouble = indir2_index(lastIndex) == 0 ? true : false;
    int32_t indirectIndex = indir_index(lastIndex); //which indriect block
    int32_t directIndex = direct_index(lastIndex); //which block

    //if we are still at direct block
    if(indirectIndex == -1)
    {
    	free_block(oi->oi_direct[lastIndex]);
        oi->oi_direct[lastIndex] = 0;
    }

    //if we are at indrect block region
    else if(indirectIndex == 0)
    {
    	uint32_t* indirectBlockArray = (uint32_t*)ospfs_block(oi->oi_indirect);
    	free_block(indirectBlockArray[directIndex]); 
    	indirectBlockArray[directIndex] = 0;
    	if(directIndex == 0) //deleted the first indrect entry leads to delete of entire indirect block
    	{
    ...
    {
		uint32_t* doubleIndirectArray = (uint32_t*) ospfs_block(oi->oi_indirect2);
        uint32_t* indirectBlockArray = (uint32_t*) ospfs_block(doubleIndirectArray[indirectIndex]);

        //remove data block
        free_block(indirectBlockArray[directIndex]);
        indirectBlockArray[directIndex] = 0;

        //if deleted the only entry in indirect block, delete that indirect block from the double indrect block
        if (directIndex==0) 
        {
            free_block(doubleIndirectArray[indirectIndex]);
            doubleIndirectArray[indirectIndex] = 0;
        }

        //if deleted the only entry in double indrect block, delet the entire double indirect block
        if (directIndex==0 && indirectIndex==0) {
            free_block(oi->oi_indirect2);
            oi->oi_indirect2 = 0;
        }
    }
    //reaching here means block is sucessfully freed
    oi->oi_size -= OSPFS_BLKSIZE;
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	/* EXERCISE: Your code here */
    uint32_t fszie = oi->oi_size;
    
    if(*f_pos+count >fszie)
    {
	...
	while (amount < count && retval >= 0) {
		...
		//	Use this function to look up the blocks that are part of a file's contents.
		uint32_t n;
		char *data;
		...
		// count: toayl # of bytes needed to copy
		// n: # of bytes copied in THIS round
		// amount: cumalitive count of bytes copied so far 
        size_t startpos = (size_t) (*f_pos % OSPFS_BLKSIZE);

        n = OSPFS_BLKSIZE - startpos; //amount of bytes copied from this block

        //if what we need to copy does not fill up the entire block, we don't want
        //n to be the complement of the blockSize
       	if(count - amount <= n)
       		n = count - amount;

        if (copy_to_user(buffer, data + startpos, n))
        {
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
        ...
        size_t startpos = (size_t) (*f_pos % OSPFS_BLKSIZE);
		n = OSPFS_BLKSIZE - startpos;
        
        if(count - amount <= n)
            n = count - amount;
    
        if (copy_from_user(data + startpos, buffer , n)!=0)
        {
            retval = -EFAULT;
            goto done;
        }
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
    ...
    //check whether there is empty entry
    for (offsetdirect=0; offsetdirect <dir_oi->oi_size; offsetdirect+=OSPFS_DIRENTRY_SIZE)
    {
        d_entry=ospfs_inode_data(dir_oi, offsetdirect);
        if(d_entry->od_ino == 0)
        {
            return d_entry;
        }
    }
    // there's no empty entries, add a block
    
    adding = add_block(dir_oi);
    
    if (adding <0) return ERR_PTR(adding);
    else if (adding==0)
    {
    	//notice after for loop offsetdirect is exactly dir_oi->oi_size,
    	//which becomes the start of newly added block
        d_entry = (ospfs_direntry_t*) ospfs_inode_data(dir_oi, offsetdirect);
    }
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
    ...
    if (src_dentry->d_inode->i_ino ==0 || src_dentry->d_name.len==0)
        return -EINVAL;
    
    ospfs_inode_t *source_oi=ospfs_inode(src_dentry->d_inode->i_ino);
    //if the source file is the directory
    if (source_oi->oi_ftype == OSPFS_FTYPE_DIR)
        return -EINVAL;
    ...
    // if a file has the same name already existed in the given directory
    ospfs_inode_t *dest_oi=ospfs_inode(dir->i_ino);
    
    if (find_direntry(dest_oi, dst_dentry->d_name.name, dst_dentry->d_name.len))
        return -EEXIST;
    
    //if the disk is full & the file can't be created; create the new hardlink file
    ospfs_direntry_t* newdentry = create_blank_direntry(dest_oi);
    if (IS_ERR(newdentry)){
        return PTR_ERR(newdentry);
    }
    
    memcpy(newdentry->od_name, dst_dentry->d_name.name, dst_dentry->d_name.len); //copy the information
    newdentry->od_ino=src_dentry->d_inode->i_ino; //use the same inode for hard link
    newdentry->od_name[dst_dentry->d_name.len] = '\0';
    
    //then increase the link number on the original file
    source_oi->oi_nlink ++;
    return 0;
}
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	if(dentry->d_name.len > OSPFS_MAXNAMELEN)
		return -ENAMETOOLONG;
	//check duplicate in curr directory
	if (find_direntry(dir_oi, dentry->d_name.name, dentry->d_name.len))
        return -EEXIST;

	//create an empty directory entry in our directory inode
	...
	//allocate an inode assciated with the new directory
	//by traversing the inode block and find the first empty inode 
	ospfs_inode_t* newInode;
	//Inode number 0 is illegal, and inode number 1 is reserved for the root directory.
	for(;entry_ino < ospfs_super->os_ninodes; entry_ino++)
	{
		newInode = ospfs_inode(entry_ino);
		if(newInode->oi_nlink == 0)	
			break;
	}
    ...
    int i=0;
    for (; i<OSPFS_NDIRECT; i++) //zero out drect block, in case it was previously a sym link
        newInode->oi_direct[i] = 0;

    //connect newInode with new directory entry
	od->od_ino = entry_ino;
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	for(;entry_ino < ospfs_super->os_ninodes; entry_ino++)
	{
		newInode = (ospfs_symlink_inode_t*)ospfs_inode(entry_ino);
		if(newInode->oi_nlink == 0)	
			break;
	}
	if(entry_ino == ospfs_super->os_ninodes)
    ...
    newInode->oi_nlink = 1;
    strcpy(newInode->oi_symlink, symname);
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	// Exercise: Your code here.
	//allocate an temp buffer becasue we don't want to mess with the original string
	char* arguement = kmalloc(sizeof(char)*(strlen(oi->oi_symlink+1)),0);
 	strcpy(arguement,oi->oi_symlink);
	char* dest = arguement;
	//printk("arguement is %s\n",arguement);
	if (strchr(oi->oi_symlink,':') != 0)
	{
		int semiIndex = 5;
		while (arguement[semiIndex] != ':')
			semiIndex++;
		arguement[semiIndex] = '\0';
		if (current->uid == 0)
		{
			//printk("is root!: ");
			dest = arguement + 5;
